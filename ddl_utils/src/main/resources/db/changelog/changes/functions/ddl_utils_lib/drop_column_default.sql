CREATE OR REPLACE FUNCTION ddl_utils_lib.drop_column_default(
...
$$
BEGIN
    -- The column name is an identifier, so it is quoted with %I and the
    -- fragment contains no caller-supplied SQL.
    PERFORM ddl_utils_lib.alter_table(
            i_schema_name => i_schema_name,
            i_table_name => i_table_name,
            i_alter_table_fragment => pg_catalog.format('ALTER COLUMN %I DROP DEFAULT', i_column_name),
